...
if queue_enable:
    ...
    css = '\n'.join(
        '<link href="{}" rel="stylesheet">'.format(urljoin('/608queue/spring18/', 'css', sheet))
        for sheet in stylesheets
    )

    js = '\n'.join(
        '<script src="{}"></script>'.format(urljoin('/608queue/spring18/', 'js', script))
